...
- A triggered ability with a required card target used to fall back to the triggering card when nothing legal could be picked, even when that card was not a legal target (wrong side, Stealth). It now resolves with no target: its `who: "target"` effects are skipped and the rest still run.
- A triggered ability whose card is silenced after it triggered but before it resolves is now ignored, like one whose card left the realm (a Deathrite still resolves from the cemetery).
- State-based deaths are re-checked until nothing more dies, so a unit whose Life depended on a passive from a unit that died in the same batch now dies too, whatever order the squares are checked in.
